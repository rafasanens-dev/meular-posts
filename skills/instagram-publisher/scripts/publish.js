...
export function parseArgs(argv) {
  const args = { images: [], imageUrls: [], caption: '', dryRun: false, scheduleNext: false, scheduleFor: null };
  for (let i = 2; i < argv.length; i++) {
    if (argv[i] === '--images') {
      if (i + 1 < argv.length) args.images = argv[++i].split(',').map(s => s.trim());
    }
    else if (argv[i] === '--image-urls') {
      if (i + 1 < argv.length) args.imageUrls = argv[++i].split(',').map(s => s.trim());
    }
    else if (argv[i] === '--caption') {
      if (i + 1 < argv.length) args.caption = argv[++i];
    }
    else if (argv[i] === '--dry-run') args.dryRun = true;
    else if (argv[i] === '--schedule-next') args.scheduleNext = true;
    else if (argv[i] === '--schedule-for') {
      // Unix timestamp (seconds) or ISO date string
      if (i + 1 < argv.length) args.scheduleFor = argv[++i];
    }
  }
  return args;
}
...
export async function createCarouselContainer(userId, childIds, caption, accessToken, scheduledUnixTs = null) {
  const params = new URLSearchParams({
    media_type: 'CAROUSEL',
  ...
  if (scheduledUnixTs) {
    ...
    params.set('scheduled_publish_time', String(scheduledUnixTs));
  }
  const res = await fetch(`${IG_BASE}/${userId}/media?${params}`, { method: 'POST' });
...
async function main() {
  const { images, imageUrls: preHostedUrls, caption, dryRun, scheduleNext, scheduleFor } = parseArgs(process.argv);

  const usingPreHosted = preHostedUrls.length > 0;
  if (!usingPreHosted && !images.length) throw new Error('--images or --image-urls is required');
  if (!caption || caption === 'undefined') throw new Error('--caption is required and must not be empty');

  const slideCount = usingPreHosted ? preHostedUrls.length : images.length;
  if (slideCount < 2 || slideCount > 10) {
    throw new Error(`Instagram carousels require 2–10 images (got ${slideCount})`);
  }
  ...
  let imageUrls;
  if (usingPreHosted) {
    console.log(`\n🔗 Using ${preHostedUrls.length} pre-hosted image URLs (skipping imgBB upload)`);
    imageUrls = preHostedUrls;
  } else {
    ...
    imageUrls = await Promise.all(images.map(p => uploadToImgBB(p, IMGBB_API_KEY)));
  ...
  for (const [i, url] of imageUrls.entries()) {
    ...
    childIds.push(id);
    console.log(`   [${i + 1}/${imageUrls.length}] ${id}`);
  }
  ...
  // Resolve scheduled timestamp if needed
  let scheduledUnixTs = null;
  if (scheduleNext || scheduleFor) {
    const { nextSlot } = await import('./scheduler.js');
    const queuePath = new URL('../../../squads/imoveis-instagram/queue.json', import.meta.url);
    const queue = JSON.parse(readFileSync(queuePath, 'utf8'));
    if (scheduleFor) {
      scheduledUnixTs = isNaN(Number(scheduleFor))
        ? Math.floor(new Date(scheduleFor).getTime() / 1000)
        : Number(scheduleFor);
    } else {
      const slot = nextSlot(queue);
      scheduledUnixTs = slot.unixTs;
      console.log(`\n📅 Agendando para: ${slot.label} (${new Date(scheduledUnixTs * 1000).toISOString()})`);
    }
  }

  console.log('\n🎠 Creating carousel container...');
  const carouselId = await createCarouselContainer(
    INSTAGRAM_USER_ID, childIds, caption, INSTAGRAM_ACCESS_TOKEN, scheduledUnixTs
  );
  ...
  if (scheduledUnixTs) {
    // Scheduled post — update queue.json
    const { addToQueue } = await import('./scheduler.js');
    const queuePath = new URL('../../../squads/imoveis-instagram/queue.json', import.meta.url);
    const queue = JSON.parse(readFileSync(queuePath, 'utf8'));
    addToQueue(queue, { postId, scheduledUnixTs, caption: caption.slice(0, 60) });
    const { writeFileSync } = await import('node:fs');
    writeFileSync(queuePath, JSON.stringify(queue, null, 2));
    const scheduledDate = new Date(scheduledUnixTs * 1000).toLocaleString('pt-BR', { timeZone: 'Asia/Tokyo' });
    console.log(`\n✅ Agendado com sucesso!`);
    console.log(`   Post ID: ${postId}`);
    console.log(`   Publica em: ${scheduledDate} (JST)`);
  } else {
